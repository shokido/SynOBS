...
import numpy as np
import os
dir_work="../../PREDICTION/OPF_G3Argo/"
...
type_name="daily"
fflag_tail="_OPFG3ARGO_"+exp_name+".nc"
dts_ini=[]
...
for inum in range(0,num_ini):
    ...
    #yyymm_fcst=[ for i in range(0,dt_end-dt_start).days)]
    nskip=5
    ...
    fname_out=dir_out+"TSREF_"+pred_flag+fflag_tail
    fnames_in=[dir_argo+"ArRef"+str(i) for i in yyyymm_fcst_unique]

    # Read Argo position data
    lon_out=[];lat_out=[];time_out=[]
    WMO_number=[];OBS_number=[]
    obs_num_start=50000
    for ifile in range(0,len(fnames_in)):
        ...
        for iline in range(0,len(lines)):
            ...
            if ((dt_tmp >= dt_start) and (dt_tmp<dt_end)):
                ...
                lon_out.append(lon)
                lat_out.append(lat)
                time_out.append((dt_tmp-dt_start).days)
    ...
    nc_out.createVariable("leadtime","float",[numname])
    ...
    nc_out.variables["leadtime"].long_name="leadtime"
    ...
    nc_out.variables["leadtime"][:]=np.asarray(time_out)
